...
# print(nan_rows)
# 删除
# print(data[(data['label_phase']=='1') | (data['label_phase']=='2')])
result = data.drop([1434, 3238,114,115,116,117], axis=0)

labels = result['sheet_name'].unique().tolist()

# 创建标签到数字编码的映射
label2idx = {label: idx for idx, label in enumerate(labels)}

# 将标签转换为数字编码
result['label_region'] = result["sheet_name"].map(label2idx)
# result['label_region'] = result["sheet_name"].apply(lambda x: label2idx[x])

print(result['label_region'].unique())
print(result)
...
def SG(data):
    ...
    for row in rows:
        data.loc[row, cols] = savgol_filter(data.loc[row, cols], window_length=21, polyorder=2)
    return data

result=SG(result)
    
features=result.loc[:,'2':'3602']
targets = pd.DataFrame(result['label_region'])
